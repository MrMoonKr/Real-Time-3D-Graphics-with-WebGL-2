...
const utils = {
    ...
    /**
     * 
     * @param {Array<number>} vertices 정점 위치 목록
     * @param {Array<number>} indices 정점 인덱스 목록
     * @returns 
     */
    calculateNormals( vertices, indices ) 
    {
        const x = 0, y = 1, z = 2, normals = [] ;
        ...
        for ( let i = 0 ; i < indices.length ; i += 3 )
        {
            ...
            // p0 - p1
            v2[x] = vertices[ 3 * indices[ i + 0 ] + x ] - vertices[ 3 * indices[ i + 1 ] + x ] ;
            v2[y] = vertices[ 3 * indices[ i + 0 ] + y ] - vertices[ 3 * indices[ i + 1 ] + y ] ;
            v2[z] = vertices[ 3 * indices[ i + 0 ] + z ] - vertices[ 3 * indices[ i + 1 ] + z ] ;
    ...
    configureControls( settings, options = { width: 300 } ) 
    {
        ...
        const isFolder = v => !isAction(v) &&  typeof v === 'object' &&  ( v.value === null || v.value === undefined ) ;
        const isColor  = v => ( typeof v === 'string' && ~v.indexOf('#') ) || ( Array.isArray( v ) && v.length >= 3 ) ;
